...
public class WidgetControllerTests {
  ...
  @InjectMocks private WidgetController widgetController;
  ...
  public void when_getAllWidgets_expect_allWidgets() throws Exception {
    ...
    MvcResult result =
        mockMvc.perform(get("/v1/widgets")).andExpect(status().isOk()).andDo(print()).andReturn();

    List<Widget> parsedResult =
        objectMapper.readValue(
            result.getResponse().getContentAsString(), new TypeReference<List<Widget>>() {});
